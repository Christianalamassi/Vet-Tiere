...
class UserForm(forms.ModelForm):
    ...
    oclock = forms.ChoiceField(
        choices=UserInfo.time_options, widget=forms.Select(attrs={
    ...
    def clean_date(self):
        ...
        if date < timezone.now().date():
            raise ValidationError(
                'Please make sure that your date is in the future.'
            )
        return date
    ...
    def clean_oclock(self):
        ...
        date = self.cleaned_data.get('date')
        if (UserInfo.objects.filter(oclock=oclock).exists()
                and UserInfo.objects.filter(date=date).exists()):
            raise forms.ValidationError(
                    "Oops! This appointment has been taken"
                )
        return oclock
